...
unsigned long co2MeasurementInterval = 5000;     // Time to wait between reads (in milliseconds).
...
MyMessage msgBmePress(CHILD_ID_BME680baro, V_VAR4);
...
MyMessage msgBmeGasPrefix(CHILD_ID_BME680gas, V_UNIT_PREFIX);  // Custom unit message.
MyMessage msgBmePressPrefix(CHILD_ID_BME680baro, V_UNIT_PREFIX);  // Custom unit message.
MyMessage msgBmeIAQPrefix(CHILD_ID_BME680iaq, V_UNIT_PREFIX);  // Custom unit message.
MyMessage msgBmeIAQAPrefix(CHILD_ID_BME680iaqa, V_UNIT_PREFIX);  // Custom unit message.
...
void presentation()
{
  ...
  present(CHILD_ID_BME680baro, S_CUSTOM);
...
void setup()
{
  ...
  mySerial.write(0XFD);
  delay(500);
  send(msgBmeGasPrefix.set("ohm"));  // Set custom unit.
    delay(100);
  send(msgBmePressPrefix.set("Pa"));  // Set custom unit.
    delay(100);
  send(msgBmeIAQPrefix.set("IAQ"));  // Set custom unit.
    delay(100);
  send(msgBmeIAQAPrefix.set("IAQAccuracy"));  // Set custom unit.
    delay(100);
}
